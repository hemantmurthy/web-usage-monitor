...
<div id="page">
	...
	<div id="nav">
		...
		<ul>
			<li class="special"><a href="datestats?date=<c:out value="${requestScope['date']}"/><c:if test="${not empty inclnoncat}">&inclnoncat=<c:out value="${inclnoncat}"/></c:if>"><c:out value="${requestScope['dateFormatted']}"/></a></li>
		<c:forEach var="interval" items="${IN_MEMORY_RECORDER_DATE_INTERVAL_STATS}">
			<li <c:if test="${interval.value eq param['int']}">class="selected"</c:if>><a href="intervalstats?int=<c:out value="${interval.value}"/><c:if test="${not empty inclnoncat}">&inclnoncat=<c:out value="${inclnoncat}"/></c:if>"><c:out value="${interval.key}"/></a></li>
		</c:forEach>
		</ul>
		</div>
	</div>
